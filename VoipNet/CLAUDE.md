...
- `rtp/video.rs`: H.264/VP8 RTP packetisation and frame reassembly. A call with `video: true` gets a second `MediaSession` (`Call::video`) put in video mode with `enable_video`, negotiated as its own `m=video` line by `negotiate_video_offer`/`apply_video_answer`; frames go in and out whole through `send_video_frame` and `MediaSink::on_video_frame`. Encoding is the application's job (PLAN 1.3).
- `rtp/packet.rs` also builds and parses RTCP: SR/RR with report blocks. `media/session.rs` sends one a second into the call and every 4 s, and feeds the peer's numbers (loss, jitter, RTT) to `AudioCodec::set_network_quality`, which is how Opus adapts.
