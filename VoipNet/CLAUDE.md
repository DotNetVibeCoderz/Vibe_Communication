...
- `media/conference.rs`: mix-minus audio plus video routing. A shared screen (`a=content:slides`) skips the layout entirely — `forward_conference_screen` sends it to every other participant that has a stream for one, since a screen is not in competition with a face. Video is forwarded, never mixed: `video_targets` picks the active speaker (energy measured in `contribute`, with hold and margin) or the pinned call, and holds a viewer back until a keyframe arrives. `SinkAdapter::on_video_frame` calls `Inner::forward_conference_video`, which sends the frame on each viewer's own video session.
- `rtp/quality.rs`: burst/gap classification (RFC 3611) fed from the playout loop.
- `rtp/video.rs`: H.264/VP8 RTP packetisation and frame reassembly. Simulcast (RFC 8853) goes both ways: `VideoTrack::layers` keeps one depacketiser per `a=rid`, read from the RTP header extension (`RtpPacketRef::rid`) or, once a browser stops labelling, from the SSRC it first arrived on; `choose_layer`/`best_layer` keep the encoding that fits the receivers' REMB and ask for a keyframe on every change. Sending is `EndpointConfig::video_encodings` → `a=rid:… send`/`a=simulcast:send` on the `main` stream plus `send_video_frame_as`, which writes the encoding name into each packet's one-byte extension (`RtpHeader::write_with`). A call with `video: true` gets a `MediaSession` per video stream (`Call::videos`, in m-line order: `main`, then `slides` for a screen share) put in video mode with `enable_video`, negotiated by `negotiate_video_offers`/`apply_video_answer`; frames carry their stream's `a=content` label through `send_video_frame` and `MediaSink::on_video_frame`. A withdrawn stream keeps its m-line (port 0) so both sides keep counting alike. Encoding is the application's job (PLAN 1.3).
...
- `VoipNet.Audio`: OpenAL devices (`CallAudioBridge`), `CallRecorder` (WAV/MP3 via NAudio.Lame, WAV fallback; video calls go to `Mp4Writer` or `AviWriter` through `ICallVideoWriter`, neither re-encoding — MP4 takes H.264 only and gives each frame the duration its RTP timestamps imply, AVI takes VP8 too at one measured frame rate, and the audio is PCM in both), `VoiceActivityDetector`, `AudioResampler`.
- `VoipNet.Video`: H.264 through the platform's own codec. `MediaFoundation/Mf.cs` calls IMFTransform, IMFMediaType, IMFSample, IMFMediaBuffer and ICodecAPI by vtable slot (no COM interop layer, no dependencies), so a wrong slot number is an access violation — the slot comments are the contract. The encoder tries a card's own sync MFT first (`MFTEnumEx`) and falls back to the software CLSID when it refuses system-memory NV12, which is usual — `IVideoEncoder.Implementation` says which answered. It applies its ICodecAPI settings before either media type — after `SetOutputType` they are silently ignored, which is how the GOP size and low-latency flag went unnoticed for a while — then sets its output type before its input type, runs low-latency CBR for a camera and quality-targeted rate control for a screen (`VideoContent.Detail`), and answers a PLI through `ICodecAPI::SetValue(CODECAPI_AVEncVideoForceKeyFrame)`; the decoder learns the picture size from `MF_E_TRANSFORM_STREAM_CHANGE` and de-strides NV12 on the way out. `MediaFoundationCamera` reads a camera through IMFSourceReader (pull, one picture per `Read()`, NV12 out whatever the device speaks, `IMFMediaSource::Shutdown` on dispose so the light goes out). `Windows/GdiScreenCapture` copies the screen with StretchBlt: correct everywhere, but the per-copy cost is the display driver's to set and can be ~300 ms on a virtual display whatever the rectangle size (measured), so the frame rate asked for is a ceiling. Windows only; `VideoCodecs.IsH264Available` and `VideoCapture.IsSupported` are the checks.
...
- `VoipNet.Enterprise`: `Recording/ConferenceRecorder` (the only place the SDK decodes and re-encodes: a decoder per participant on its own thread, `VideoCompositor` on a fixed clock, one encoder, `Mp4Writer`, voices mixed 20 ms at a time), `IvrFlow`/`IvrRunner`, `CallCenterService` (reserves agent, calls them, bridges legs in a conference; listen-only supervisors are put on hold from the bridge side, not muted), `RecordingService`, `CrmToolset`.
