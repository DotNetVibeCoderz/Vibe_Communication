...
| Video recording to MP4/AVI · perekaman audio + video ke MP4/AVI | 🟡 | `RecordingFormat.Mp4` writes the call's own H.264 with each frame's RTP timestamp as its duration, and `RecordingFormat.Avi` writes H.264 or VP8 at one measured frame rate; neither re-encodes anything. The audio is PCM in both, because the SDK carries no AAC encoder · MP4 dan AVI sudah bisa tanpa encode ulang; audio masih PCM karena belum ada encoder AAC |
