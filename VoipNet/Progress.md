...
| Simulcast (RFC 8853/8852) | ✅ | an offer of several encodings is answered with all of them and the stream-id extension; each is reassembled and measured separately, and the one that fits what the receivers report (REMB) is forwarded, with a keyframe asked for on every change. Sending simulcast is not implemented · menerima dan memilih di antara encoding simulcast |
