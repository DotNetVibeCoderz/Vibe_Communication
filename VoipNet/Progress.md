...
| Suite | Result · Hasil |
| --- | --- |
| Rust engine `cargo test --lib` | **135 passed** · lulus — codecs, SIP parser, digest auth, SDP, jitter buffer, SRTP (RFC 3711 and RFC 7714 GCM vectors), DTLS-SRTP handshake, ICE agent (nomination, role conflict, peer-reflexive, restart, consent), STUN, SCTP data channels, ZRTP key agreement, simulcast layer choice, WebSocket framing, loopback media, full SIP call flows over UDP/TLS/WS/WSS, FFI |
...
| Grafana dashboard | ✅ | `deploy/grafana/voipnet-queues.json` over the stored calls: traffic, service level against a chosen target, abandon rate, waits and talk time, outcomes, agents and out-of-hours demand · dashboard Grafana untuk riwayat antrean |
...
| Simulcast (RFC 8853/8852) | ✅ | both ways. Received: an offer of several encodings is answered with all of them and the stream-id extension; each is reassembled and measured separately, and the one that fits what the receivers report (REMB) is forwarded, with a keyframe asked for on every change. Sent: `VideoEncodings` puts `a=rid:… send` and `a=simulcast:send` in the offer and `SendVideoFrameAs` labels each packet with its encoding, for the application to encode each size · dua arah: menerima, memilih, dan mengirim beberapa encoding |
...
| Item | Status | Notes · Catatan |
| --- | --- | --- |
| Video call support — voice + video in one SIP session · panggilan suara + video dalam satu sesi SIP | 🟡 | `VoipClientOptions.Video` offers and accepts an `m=video` stream next to audio: a second RTP stream per call with its own port, SDES/DTLS-SRTP and ICE, H.264 (RFC 6184 FU-A/STAP-A) and VP8 (RFC 7741) packetisation with frame reassembly and keyframe detection, `VoipCall.SendVideoFrame` and `VideoFrameReceived`, plus keyframe requests in both directions (RTCP PLI/FIR, sent automatically when a frame loses packets). `VoipNet.Video` encodes and decodes H.264 and reads a camera through the platform codec (Media Foundation; Windows only so far), so an application can hand over pixels, or ask for them, rather than carrying a codec of its own; `VideoCapture.OpenPattern` gives a machine with no camera something to send · engine plus `VoipNet.Video` untuk encode/decode H.264 dan membaca kamera lewat codec platform (baru Windows) |
| Video conferencing with layout control · konferensi video multipihak dengan kontrol layout | 🟡 | the bridge forwards one participant's video to everyone else — the active speaker (picked from the audio it already mixes, with hold and margin) or a pinned call — and holds a switch until the new source sends a keyframe. For a mixed grid, `VideoCompositor` in `VoipNet.Video` lays decoded pictures out (grid, picture-in-picture, spotlight) for an application to encode and send as one stream; the engine itself forwards rather than mixes · bridge meneruskan video pembicara aktif atau peserta yang dipin; `VideoCompositor` menyusun grid untuk aplikasi yang mau mencampur sendiri |
| Screen sharing (desktop & web) · berbagi layar (desktop & web) | 🟡 | `ShareScreen()` offers a second `m=video` marked `a=content:slides` in a re-INVITE, frames carry the label in both directions, and `StopScreenShare()` withdraws it with port 0; `VideoCapture.OpenScreen` copies the screen (Windows, through GDI; the rate a driver allows varies enormously) and `VoipNet.Video` encodes it; in a conference a shared screen is forwarded to everyone at once, and the meeting sample shares from the browser with `getDisplayMedia`. Desktop Duplication, ScreenCaptureKit and PipeWire are not wired up · stream layar kedua sudah dinegosiasikan dan dilabeli; `VideoCapture.OpenScreen` menyalin layar di Windows |
| Video recording to MP4/AVI · perekaman audio + video ke MP4/AVI | 🟡 | `RecordingFormat.Mp4` writes the call's own H.264 with each frame's RTP timestamp as its duration, and `RecordingFormat.Avi` writes H.264 or VP8 at one measured frame rate; neither re-encodes anything. The audio is PCM in both, because the SDK carries no AAC encoder. `ConferenceRecorder` records a whole conference as one picture — every participant decoded, composed into a grid, encoded once, voices mixed — which needs a platform codec and H.264 participants · MP4 dan AVI tanpa encode ulang; `ConferenceRecorder` merekam seluruh ruangan sebagai satu gambar |
...
| Amazon Polly / Transcribe | 🟡 | `AmazonPollyTextToSpeech` and `AmazonTranscribeSpeechToText`: streaming transcription over a web socket signed with SigV4 query parameters, audio and transcripts in AWS's event stream framing, no AWS SDK. Tested against a stand-in that speaks the same protocol — there is no AWS account here, so it has not been run against the service itself · Polly dan Transcribe sudah ada tanpa AWS SDK; Transcribe baru diuji dengan tiruan protokolnya, belum ke layanan aslinya |
...
- Video calls record to MP4 or AVI (`RecordingFormat.Mp4`/`Avi`, `voipnet sip call --record out.mp4`): the peer's own frames next to PCM audio. MP4 keeps each frame's own timing from the RTP timestamps and holds H.264; AVI stores one measured frame rate and also takes VP8. · Panggilan video bisa direkam ke MP4 atau AVI beserta audio PCM, tanpa encode ulang.
