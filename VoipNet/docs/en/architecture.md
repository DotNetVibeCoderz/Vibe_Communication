...
**Data channels (`media/sctp.rs`).** With `DataChannels` on, the call carries an `m=application` stream whose DTLS tunnel holds an SCTP association (RFC 8261/8831). The module is sans-IO like the ICE agent: packets in, events out, with the association handshake, ordered reliable DATA with SACKs and retransmission, message fragmentation and the DCEP handshake that names a channel (RFC 8832). It has no multi-homing, no partial reliability and no congestion control beyond a bounded amount of unacknowledged data — chat and small files do not need them. Messages reach the application through `DataMessageReceived`.
