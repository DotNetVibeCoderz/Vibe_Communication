...
- **Screen sharing** for desktop and web apps: capture a screen, window or region (Windows Graphics Capture, ScreenCaptureKit, PipeWire), send it as a second video stream (`a=content:slides`, BFCP-style floor control for SIP endpoints), content-optimised encoding (high resolution, low frame rate); browser sharing via `getDisplayMedia` once WebRTC interop (1.1) lands; Softphone and Blazor samples. · **Berbagi layar** untuk aplikasi desktop/web: tangkap layar, jendela, atau area, kirim sebagai stream video kedua dengan floor control, encoding yang dioptimalkan untuk konten; di browser lewat `getDisplayMedia` setelah interop WebRTC (1.1).
