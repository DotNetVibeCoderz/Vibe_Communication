...
- **Video call support** — voice + video in one SIP session: `m=video` offer/answer next to audio, a second RTP stream per call, H.264 (RFC 6184) and VP8 (RFC 7741) packetisation with frame assembly, and the .NET API `VoipCall.SendVideoFrame`/`VideoFrameReceived` (all done); still to do: keyframe requests (RTCP PLI/FIR), bandwidth estimation and adaptive bitrate, lip sync from RTCP sender reports, camera capture and decode/encode through platform codecs (Media Foundation, VideoToolbox, VA-API/FFmpeg). · **Dukungan panggilan video** — suara + video dalam satu sesi SIP: offer/answer `m=video`, stream RTP kedua per panggilan, paketisasi H.264/VP8, jitter buffer dan perakitan frame, permintaan keyframe (PLI/FIR), estimasi bandwidth dan bitrate adaptif, sinkronisasi bibir; API .NET untuk frame video, kamera, serta encode/decode via codec platform.
